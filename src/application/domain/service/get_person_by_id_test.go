...
func TestGetPersonByIDExecute(t *testing.T) {
	...
	for _, test := range tests {
		...
		personAdapterMock.On("GetPersonByID", mock.Anything).Return(test.responseSuccessAdapter, test.responseErrorAdapter)

		getPersonByIDService := NewGetPersonByIDService(personAdapterMock)
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.inputData != nil {
				result, err = getPersonByIDService.Execute(test.inputData)
			} else {
				result, err = getPersonByIDService.Execute()
			}
