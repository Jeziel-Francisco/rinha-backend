...
func TestPersonCreateExecute(t *testing.T) {
	...
	tests := []scenarions{
		...
		},
		{
			name:               "invalid params",
			inputData:          1,
			outputServiceError: errors.NewInvaliParamApiError("person"),
		},
		{
		...
		},
		{
			name:                   "person created",
			inputData:              entities.Person{},
			outputServiceSuccess:   "123",
			responseSuccessAdapter: &entities.Person{ID: "123"},
		},
	}

	for _, test := range tests {
		personAdapterMock := person.NewPersonDatabaseAdapterMock()
		personAdapterMock.On("Create", mock.Anything).Return(test.responseSuccessAdapter, test.responseErrorAdapter)

		checkExistsNicknameService := NewPersonCreateService(personAdapterMock)
